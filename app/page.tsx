...
export default async function HomePage() {
  const stations = await getStationsByCountry({
    offset: 0,
    limit: 32,
    country: "IN",
  }).catch((error: unknown) => {
  ...
  return (
    <div className="flex h-full flex-1 flex-col gap-6">
      <header className="flex flex-col gap-3 pr-24 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-400 sm:text-sm">
            Multi-language internet radio from around the world.
          </p>
        </div>
      ...
      <section className="flex-1 pb-6">
        <h2 className="sr-only">Available Radio Stations</h2>
        <StationsPageClient
